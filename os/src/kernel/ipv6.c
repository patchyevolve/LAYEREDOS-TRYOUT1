...
static int ipv6_handler_registered = 0;

#define ipv6_initialized (get_current_ns()->ipv6_initialized)
#define ipv6_dispatch (get_current_ns()->ipv6_dispatch)
#define GLOBAL_IPV6 (get_current_ns()->global_ipv6)
#define LINK_LOCAL (get_current_ns()->link_local)
#define ipv6_mcast_groups (get_current_ns()->ipv6_mcast_groups)
...
void ipv6_set_addr(const uint8_t* addr) {
    ...
    kprintf("[IPv6] Global address set\n");
}
...
int ipv6_mcast_join(const uint8_t* group_addr) {
    if (group_addr[0] != 0xFF) return ERR_INVAL;
    ...
    for (int i = 0; i < IPV6_MAX_MCAST_GROUPS; i++) {
        if (!ipv6_mcast_groups[i].used) {
            ...
            kprintf("[IPv6] Joined multicast group\n");
